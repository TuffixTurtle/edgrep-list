...
int getcopy(void);  int getfile(void);  char *getline_blk(unsigned int tl);  int getnum(void); int getsub(void); int getch_(void);
int gettty(void);  int gety(void);  void global(int k);  void init(void);
void join(void);  void move_(int cflag);  void newline(void);  void onhup(int n);
...
void readfile(const char* c); void search(const char* c);  void search_file(const char* filename, const char* searchfor);
void setwide(void);  void setnoaddr(void);   void substitute(int inglob); void ungetch_(int c);
jmp_buf  savej;
char grepbuf[GBSIZE];
void greperror(char);  void grepline(void);

char buf [BUFSIZ]; size_t bufp = 0;
int getch_(void) {return (bufp > 0)? buf[--bufp]:getchar();}
void ungetch_(int c){
  if (buf >= BUFSIZ) { fprintf(stderr, "ungetch: too many characters\n"); return;}
  buf[bufp++] = c;
}
...
void commands(void) {  unsigned int *a1;  int c, temp;  char lastsep;
  for (;;) {
    ...
    case 'c': newline(); rdelete(addr1,addr2); append(gettty, addr1-1); continue;
    case 'd':  newline();  rdelete(addr1,addr2);  continue;
    ...
    case 'k':    if ((c = getchr()) < 'a' || c > 'z') { error(Q); }  newline();
                names[c-'a'] = *addr2 & ~01;  anymarks |= 01;  continue;
    case 'l':   listf++;
    case 'm':   move_(0);  continue;
    case 'n':   listn++;  newline();  print();  continue;
    case 'r':   filename(c);
    case 's':     substitute(globp!=0);  continue;
    case 't':   move_(1);  continue;
    case 'u':    newline(); if ((*addr2&~01) != subnewa) { error(Q); }  *addr2 = subolda;
                dot = addr2; continue;
    case 'v':   global(0);  continue;
    case 'W':   wrapp++;  case 'w': setwide();
      ...
      if (dol > zero) { putfile(); }   if (addr1 <= zero+1 && addr2 == dol) { fchange = 0; }
                ...
                case '=':   setwide();  newline();  count = addr2 - zero; putchr_('\n'); continue;
...
void add(int i) {  if (i && (given || dol>zero)) {  addr1--;  addr2--;  } newline();  append(gettty, addr2); }
...
int advance(char *lp, char *ep) {  char *curlp;  int i;
  for (;;) {
    switch (*ep++) {
      ...
      case CCL|STAR:
      case NCCL|STAR:  curlp = lp;  goto star;
...
int compsub(void) {  int seof, c;  char *p;  if ((seof = getchr()) == '\n' || seof == ' ') { error(Q); }
 ...
 for (;;) {
   ...
   if (c == '\\') { c = getchr() | 0200; }
   if (c == '\n') {
     if (globp && globp[0]) { c |= 0200; }   else { peekc = c;  pflag++;  break; }
   }
...
void join(void) {  char *gp, *lp;  unsigned int *a1;   gp = genbuf;
...
void move_(int cflag) {  unsigned int *adt, *ad1, *ad2;
...
void readfile(const char* c) {
  setnoaddr(); if (vflag && fchange) { fchange = 0;   }
    filename(*c);  init();
  addr2 = zero;  if ((io = open((const char*)file, 0)) < 0) { lastc = '\n';
error(file); }
  setwide();  ninbuf = 0; append(getfile, addr2);
fchange = *c;
}
...
void substitute(int inglob) {  int *mp, nl;  unsigned int *a1;  int gsubf = compsub(), n = getnum();
 for (a1 = addr1; a1 <= addr2; a1++) {
   if (execute(a1)){  unsigned *ozero;  int m = n;
     do {  int span = (int)(loc2 - loc1);
        if (--m <= 0) {
          dosub();  if (!gsubf) { break; }  if (span==0) { if (*loc2=='\0') { break; }  loc2++;  }
        }
